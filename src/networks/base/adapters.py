...
class BertAdapter(nn.Module):
    ...
    def forward(self,x):
        ...
        # h = batch_size x bert_adapter_size = batch_size x 2000
        h=self.activation(self.fc2(h))
        # h = batch_size x bert_hidden_size = batch_size x 768
...
class BertAdapterMask(BertAdapter):
    ...
    def __init__(self, config):
        ...
        # embedding information of task t-th
    ...
    def forward(self,x,t,s):
        """
        x : embedding input: batch_size x 768 
        t : task index : int
        s : seed training ; float
        """

        gfc1,gfc2=self.mask(t=t,s=s)
        h = self.get_feature(gfc1,gfc2,x)
        # x ban đầu đi qua một cái up dimesion layer lên 2000 
        # sau đó down dim về ban đầu (768)
        # h = batch_size x 768
        return x + h


    def get_feature(self,gfc1,gfc2,x):
        # step 1: feed hidden state into updim: 
        h=self.activation(self.fc1(x))
        # h = batch_size x bert_adapter_size = batch_size x 2000

        # step 2: fead h into mask 1 
        h=h*gfc1.expand_as(h)
        # h = batch_size x 2000

        # step 3: feed h into mask 2
        h=self.activation(self.fc2(h))
        # activation: sigmoid 
        # h = batch_size x bert_hidden_size = batch_size x 768 
