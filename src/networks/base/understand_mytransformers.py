...
class MyBertSelfOutput(BertSelfOutput):
    ...
    BCL:
    CTR: 
    Step : h = [batch_size x 768] => Linear_transform(768, 768) => adapters ==> layer norm  => h = [batch_size x 768]
    """
...
"""
self.self : attention, 
self.output: hidden_state => adapter => layer_norm
"""
class MyBertAttention(BertAttention):
    ...
    def forward(
        ...
        # output cuar self.self [BertSelfAttention] = (context_layer, attention_probs) if output_attentions else (context_layers, )
        # trong do: context_layer la ket qua cua phep tinh attention: 
        # giả sử hidden_states = [batch_size x max_seq_length x 768] 
        # sau khi đi qua bertselfattention thì được gì???
        # trả lời : 

        #  cái BertSelfAttention là không có task specific t
        # 

        # sau khi cho qua attention block xong thi cho qua adapter 
        # còn cái  self.output = MyBertSelfOutput(config,args) là có task specific cho task t 
        if self.args.apply_bert_attention_output:
            # đối với bcl, ctr, classic thì lấy cái này
            ...
            # không phải 3 trường hợp trên thì lấy cái này
            attention_output = self.output(self_outputs[0], hidden_states,t=t,s=s)
        # attention_output = [batch_size x hidden_states]
        # ======>   self_outputs[1:] is a `set`
        # self_outputs : attention_probs 
        outputs = (attention_output,) + self_outputs[1:]  # add attentions if we output them
        # final outputs là kết quả của attention_output và self_outputs
        if self.args.apply_bert_attention_output:
            # return ở đây
            if self.args.build_adapter_capsule_mask: return {'outputs':outputs} # nếu ctr, bcl return ở đây
            else: return outputs # nếu classic return ở đây
        else: return outputs
...
# sau khi kết thức layer norm thứ nhất trong transformers layer thì đến MyBertOutput
...
class MyBertLayer(BertLayer):
    ...
    # [batch_size x 768] feed into => mhead_attn --> feedfoward --> adapter -> layerNorm  =====> output => new hidden states: batch_size x 768
    def __init__(self, config,args):
        super().__init__(config)
        ...
        # mybertlayer có hai phần chính, maybertattention và mybertoutout
        self.args = args
    def forward(
        self,
        hidden_states,
        ...
        # self.attention = MyBertAttention(config,args)
        if self.args.apply_bert_attention_output:
            # đôi với adapter thì apply_bert_attention_output = true và apply_bert_output = True
            if self.args.build_adapter_capsule_mask:
                #bcl, ctr
        ...
        # output của self.args.apply_bert_attention_output là `self_attention_outputs`
...
class MyBertEncoder(BertEncoder):
    def __init__(self, config,args):
        ...
        # vấn đề cuat mybert encoder ở MyBeertLayer
        self.args = args
...
class MyBertModel(BertModel):
    ...
    def forward(
        ...
        # self.get_head_mask : [None] * num_hidden_layers : not importance

        embedding_output = self.embeddings(
            input_ids=input_ids, position_ids=position_ids,  token_type_ids=token_type_ids, inputs_embeds=inputs_embeds
        )
        # embedding like bert's embedding 
        # embedding_output = [batch_size x hidden_size]
        ...
        # vấn đề chnhs vẫn ở compute_encoder_outputs
    ...
    def compute_encoder_outputs(self,
        ...
        if self.args.build_adapter_capsule_mask:
            ...
            # vấn đề chính của compute encoder outputs ở self.encode => MyBertEncoder.
